var get_user = require("../functions/get_user"),
    ...
    function get_new_user(chatId, data) {
...
module.exports = {
	...
	f: function (msg, chatId, args, data) {
        get_user.start(args[2], (user) => {
            user.evaluations.incoming.push({
                id: chatId,
                estimation: args[3]
            });

            user.evaluations.new_incoming.push({
                id: chatId,
                estimation: args[3]
            });
            user.evaluations.average_score.sum_of_ratings += Number(args[3]);
            user.evaluations.average_score.total_ratings += 1;

            if(user.evaluations.new_incoming.length == 10) {
                data({
                    text: `<i>Твою фотографию оценили 🔥</i>`,
                    keyboard: [
                        [{"text": `Подробнее 🔎`, callback_data: "show_the_incoming_list"}]
                    ],
                    chatId: args[2]
                });
            }

            if(user.evaluations.average_score.sum_of_ratings >= 25 && user.evaluations.average_score.sum_of_ratings/user.evaluations.average_score.total_ratings < 2) {
                user.other.block = true;

                data({
                    text: `<b><u>🔔 Уведомление 🔔</u></b>\n<i>Твоя анкета не нравится людям, аккаунт снят с показа (выдан теневой бан). Поменяй фотографию, если хочешь и дальше получать оценки от пользователей.</i>`,
                    keyboard: [
